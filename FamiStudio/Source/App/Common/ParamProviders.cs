            ...
            switch (instrument.Expansion)
            {
                ...
                case ExpansionType.Vrc7:
                    ...
                    paramInfos.Add(new InstrumentParamInfo(instrument, ReleaseLabel, 0, 15, (Vrc7InstrumentPatch.Infos[1].data[7] & 0x0f) >> 0)
                        { GetValue = () => { return (instrument.Vrc7PatchRegs[7] & 0x0f) >> 0; }, SetValue = (v) => { instrument.Vrc7PatchRegs[7] = (byte)((instrument.Vrc7PatchRegs[7] & (~0x0f)) | ((v << 0) & 0x0f)); instrument.Vrc7Patch = 0; instrument.Vrc7Release = (byte)(instrument.Vrc7PatchRegs[7] & 0x0f); }, IsEnabled = () => !instrument.Vrc7SustainBitSet,  TabName = "Carrier" });
                    ...
                    paramInfos.Add(new InstrumentParamInfo(instrument, SustainBitLabel, 0, 1, 0)
                        { GetValue = () => { return instrument.Vrc7SustainBitSet ? 1 : 0; }, SetValue = (v) => { instrument.Vrc7SustainBitSet = v != 0; instrument.Vrc7PatchRegs[7] = (byte)((instrument.Vrc7PatchRegs[7] & (~0x0f)) | ((v != 0 ? 0x05 : instrument.Vrc7Release) & 0x0f)); } }); 
